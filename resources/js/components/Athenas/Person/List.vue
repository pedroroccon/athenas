<template>
    <div class="persons">
        <button type="button" @click="showCreateForm" class="btn btn-primary mb-4">Adicionar pessoa</button>
        ...
        <div class="card card-body shadow-lg rounded">
            <div class="table-responsive">
                <table class="table table-hover table-nowrap">
                    ...
                    <tbody>
                        <tr v-for="(person, index) in registers.data" :key="person.id">
                            <td>{{ person.code }}</td>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getRegisters() {
                axios.get('/athenas/api/person' + '?page=' + this.page).then(response => {
            ...
            remove(index) {
                axios.delete('/athenas/api/person/' + this.registers.data[index].id).then(response => {
                    this.getRegisters()
                });
            }, 
            onPageChange(page) {
